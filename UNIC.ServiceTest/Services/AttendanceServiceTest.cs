...
namespace UNIC.ServiceTest.Services
{
    public class AttendanceServiceTest
    {
        ...
        #region VerifyAttendanceByLinkAsync

        [Fact]
        public async Task VerifyAttendanceByLinkAsync_Success_SetsPresent()
        {
            var attendance = new Attendance
            {
                EventId = 1,
                CheckInToken = "VALID123",
                AttendanceStatus = nameof(AttendanceStatus.REGISTERED),
                User = new User { Email = "user@test.com", FullName = "Test" },
                Event = new Event { EventName = "Test Event" }
            };
            _mockUnitOfWork.Setup(u => u.Attendances.GetByCheckInTokenAsync("VALID123")).ReturnsAsync(attendance);
            _mockUnitOfWork.Setup(u => u.SaveChangesAsync()).ReturnsAsync(1);

            var result = await _attendanceService.VerifyAttendanceByLinkAsync("user@test.com", "VALID123");

            result.Success.Should().BeTrue();
            attendance.AttendanceStatus.Should().Be("PRESENT");
        }

        [Fact]
        public async Task VerifyAttendanceByLinkAsync_InvalidCode_ReturnsFail()
        {
            var result = await _attendanceService.VerifyAttendanceByLinkAsync("a@b.com", "   ");
        ...
        [Fact]
        public async Task VerifyAttendanceByLinkAsync_EmailMismatch_ReturnsFail()
        {
            var attendance = new Attendance
            {
                CheckInToken = "TOKEN1",
                AttendanceStatus = nameof(AttendanceStatus.REGISTERED),
                User = new User { Email = "real@test.com", FullName = "Real" }
            };
            _mockUnitOfWork.Setup(u => u.Attendances.GetByCheckInTokenAsync("TOKEN1")).ReturnsAsync(attendance);

            var result = await _attendanceService.VerifyAttendanceByLinkAsync("wrong@test.com", "TOKEN1");

            result.Success.Should().BeFalse();
            result.Message.Should().Contain("Email không khớp");
        }

        [Fact]
        public async Task VerifyAttendanceByLinkAsync_AlreadyCheckedIn_ReturnsSuccessWithFlag()
        {
            var attendance = new Attendance
            {
                EventId = 1,
            ...
            _mockUnitOfWork.Setup(u => u.Attendances.GetByCheckInTokenAsync("TOKEN2")).ReturnsAsync(attendance);

            var result = await _attendanceService.VerifyAttendanceByLinkAsync("user@test.com", "TOKEN2");
        ...
        #region CancelRegistrationAsync — Slot Release

        [Fact]
        public async Task CancelRegistrationAsync_WithMaxAttendees_PromotesWaitlist()
        {
            var userId = Guid.NewGuid();
            var attendance = new Attendance
            {
            ...
            var eventEntity = new Event { EventId = 1, MaxAttendees = 50, RequiresApproval = false };

            _mockUnitOfWork.Setup(u => u.Attendances.GetByEventAndUserAsync(1, userId)).ReturnsAsync(attendance);
            _mockUnitOfWork.Setup(u => u.SaveChangesAsync()).ReturnsAsync(1);
            _mockUnitOfWork.Setup(u => u.Events.GetByIdAsync(1)).ReturnsAsync(eventEntity);
            _mockUnitOfWork.Setup(u => u.Events.TryDirectPromoteOldestWaitlistAsync(1, nameof(AttendanceStatus.REGISTERED))).ReturnsAsync(true);

            await _attendanceService.CancelRegistrationAsync(1, userId);

            attendance.AttendanceStatus.Should().Be(nameof(AttendanceStatus.CANCELLED));
            _mockUnitOfWork.Verify(u => u.Events.TryDirectPromoteOldestWaitlistAsync(1, nameof(AttendanceStatus.REGISTERED)), Times.Once);
            // IncrementSlot should NOT be called when someone was promoted
            _mockUnitOfWork.Verify(u => u.Events.IncrementSlotAsync(1), Times.Never);
        }

        [Fact]
        public async Task CancelRegistrationAsync_WithMaxAttendees_NoWaitlist_IncrementsSlot()
        {
            var userId = Guid.NewGuid();
            var attendance = new Attendance
            {
            ...
            _mockUnitOfWork.Setup(u => u.SaveChangesAsync()).ReturnsAsync(1);
            _mockUnitOfWork.Setup(u => u.Events.GetByIdAsync(1)).ReturnsAsync(eventEntity);
            _mockUnitOfWork.Setup(u => u.Events.TryDirectPromoteOldestWaitlistAsync(1, nameof(AttendanceStatus.REGISTERED))).ReturnsAsync(false);

            await _attendanceService.CancelRegistrationAsync(1, userId);

            // IncrementSlot SHOULD be called when no waitlist promotion happened
            _mockUnitOfWork.Verify(u => u.Events.IncrementSlotAsync(1), Times.Once);
        }

        [Fact]
        public async Task CancelRegistrationAsync_WaitlistStatus_NoSlotRelease()
        {
            var userId = Guid.NewGuid();
            var attendance = new Attendance
            {
            ...
            attendance.AttendanceStatus.Should().Be(nameof(AttendanceStatus.CANCELLED));
            // WAITLIST does NOT occupy a slot, so no slot release
            _mockUnitOfWork.Verify(u => u.Events.GetByIdAsync(It.IsAny<int>()), Times.Never);
        }
